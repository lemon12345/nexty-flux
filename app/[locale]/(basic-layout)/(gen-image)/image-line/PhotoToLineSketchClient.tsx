...
const FEATURE_KEY = "photo_to_line_sketch"; // config/featureList.ts

export default function PhotoToLineSketchClient() {
  ...
  const t = useTranslations("PhotoToLineSketch.client");

  const [sourceImage, setSourceImage] = useState<string | null>(null);
  const [sourceImageUrl, setSourceImageUrl] = useState<string | null>(null);
  const [resultImage, setResultImage] = useState<string | null>(null);
  const [prompt, setPrompt] = useState("Transform the image into a line sketch drawing.");
  ...
  const selectedModel = "photo_to_line_sketch";
  ...
  const saveFormData = useCallback(() => {
    ...
    localStorage.setItem("photo_to_line_sketch_form", JSON.stringify(formData));
  }, [prompt, aspectRatio, outputFormat, seed, guidanceScale]);
  ...
  }, [prompt, aspectRatio, outputFormat, seed, guidanceScale, saveFormData]);

  useEffect(() => {
    const savedJob = localStorage.getItem("photo_to_line_sketch_job");
    if (savedJob) {
      try {
        ...
        localStorage.removeItem("photo_to_line_sketch_job");
      }
    }

    const savedImage = localStorage.getItem("photo_to_line_sketch_image");
    if (savedImage) {
      try {
        ...
        localStorage.removeItem("photo_to_line_sketch_image");
      }
    }

    const savedFormData = localStorage.getItem("photo_to_line_sketch_form");
    if (savedFormData) {
      try {
        ...
        localStorage.removeItem("photo_to_line_sketch_form");
      }
    }

    const savedResult = localStorage.getItem("photo_to_line_sketch_result");
    if (savedResult) {
      try {
        ...
        localStorage.removeItem("photo_to_line_sketch_result");
      }
    }
  }, []);
    ...
    const pollJobStatus = async () => {
      try {
        ...
        if (result.success) {
          ...
          setJob(job);
          localStorage.setItem("photo_to_line_sketch_job", JSON.stringify(job));
  ...
  const handleEdit = useCallback(() => {
    ...
    localStorage.setItem("photo_to_line_sketch_image", JSON.stringify(imageData));
    ...
    <section className="container mx-auto px-4 py-4 mb-8">
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-lg p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col space-y-6">
          ...
          {/* Hidden prompt input - using default value for line sketch generation */}
          <input
            type="hidden"
            value={prompt}
