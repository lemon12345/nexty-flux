...
const FEATURE_KEY = "photo_to_vintage"; // config/featureList.ts

export default function PhotoToVintageClient() {
  ...
  const t = useTranslations("PhotoToVintage.client");

  const [sourceImage, setSourceImage] = useState<string | null>(null);
  const [sourceImageUrl, setSourceImageUrl] = useState<string | null>(null);
  const [resultImage, setResultImage] = useState<string | null>(null);
  const [prompt, setPrompt] = useState("Transform to Makoto Shinkai Style");
  ...
  const saveFormData = useCallback(() => {
    ...
    localStorage.setItem("photo_to_vintage_form", JSON.stringify(formData));
  ...
  useEffect(() => {
    const savedJob = localStorage.getItem("photo_to_vintage_job");
    if (savedJob) {
      try {
        ...
        localStorage.removeItem("photo_to_vintage_job");
      }
    }

    const savedImage = localStorage.getItem("photo_to_vintage_image");
    if (savedImage) {
      try {
        ...
        localStorage.removeItem("photo_to_vintage_image");
      }
    }

    const savedFormData = localStorage.getItem("photo_to_vintage_form");
    if (savedFormData) {
      try {
        ...
        localStorage.removeItem("photo_to_vintage_form");
      }
    }

    const savedResult = localStorage.getItem("photo_to_vintage_result");
    if (savedResult) {
      try {
        ...
        localStorage.removeItem("photo_to_vintage_result");
      }
    }
  }, []);
    ...
    const pollJobStatus = async () => {
      try {
        ...
        if (result.success) {
          ...
          setJob(job);
          localStorage.setItem("photo_to_vintage_job", JSON.stringify(job));
  ...
  const handleEdit = useCallback(() => {
    ...
    localStorage.setItem("photo_to_vintage_image", JSON.stringify(imageData));
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-lg p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col space-y-6">
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white py-3 rounded-lg font-medium transition-all duration-200 shadow-lg hover:shadow-xl"
            size="lg"
          >
            {isProcessing ? (
              <>
