...
const FEATURE_KEY = "photo_to_vibrant"; // config/featureList.ts

export default function PhotoToVibrantClient() {
  ...
  const t = useTranslations("PhotoToVibrant.client");

  const [sourceImage, setSourceImage] = useState<string | null>(null);
  const [sourceImageUrl, setSourceImageUrl] = useState<string | null>(null);
  const [resultImage, setResultImage] = useState<string | null>(null);
  const [prompt, setPrompt] = useState("Transform the image into linnea teal hair smiling, soft illustration style");
  ...
  const saveFormData = useCallback(() => {
    ...
    localStorage.setItem("photo_to_vibrant_form", JSON.stringify(formData));
  ...
  useEffect(() => {
    const savedJob = localStorage.getItem("photo_to_vibrant_job");
    if (savedJob) {
      try {
        ...
        localStorage.removeItem("photo_to_vibrant_job");
      }
    }

    const savedImage = localStorage.getItem("photo_to_vibrant_image");
    if (savedImage) {
      try {
        ...
        localStorage.removeItem("photo_to_vibrant_image");
      }
    }

    const savedFormData = localStorage.getItem("photo_to_vibrant_form");
    if (savedFormData) {
      try {
        ...
        localStorage.removeItem("photo_to_vibrant_form");
      }
    }

    const savedResult = localStorage.getItem("photo_to_vibrant_result");
    if (savedResult) {
      try {
        ...
        localStorage.removeItem("photo_to_vibrant_result");
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const TIMEOUT_DURATION = 2 * 60 * 1000; // 2分钟

    const pollJobStatus = async () => {
      try {
        // 检查是否超时
        if (Date.now() - startTime > TIMEOUT_DURATION) {
          console.warn("Job status polling timeout after 2 minutes");
        ...
        if (result.success) {
          ...
          setJob(job);
          localStorage.setItem("photo_to_vibrant_job", JSON.stringify(job));
  ...
  const handleEdit = useCallback(() => {
    ...
    localStorage.setItem("photo_to_vibrant_image", JSON.stringify(imageData));
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-lg p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col space-y-6">
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white py-3 rounded-lg font-medium transition-all duration-200 shadow-lg hover:shadow-xl"
            size="lg"
          >
            {isProcessing ? (
              <>
