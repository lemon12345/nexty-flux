...
const FEATURE_KEY = "photo_to_coloring_page"; // config/featureList.ts

export default function PhotoToColoringPageClient() {
  ...
  const t = useTranslations("PhotoToColoringPage.client");

  const [sourceImage, setSourceImage] = useState<string | null>(null);
  const [sourceImageUrl, setSourceImageUrl] = useState<string | null>(null);
  const [resultImage, setResultImage] = useState<string | null>(null);
  const [prompt, setPrompt] = useState("Convert image into coloring page ,Use ​​thick, bold outlines​​ and ​​uniform line weight​​ throughout. Pure white background. Minimalist.");
  ...
  const saveFormData = useCallback(() => {
    ...
    localStorage.setItem("photo_to_coloring_page_form", JSON.stringify(formData));
  }, [prompt, aspectRatio, outputFormat, seed, selectedModel]);

  useEffect(() => {
    if (
      (prompt && prompt !== "Transform to coloring page style.") ||
  ...
  useEffect(() => {
    const savedJob = localStorage.getItem("photo_to_coloring_page_job");
    if (savedJob) {
      try {
        ...
        localStorage.removeItem("photo_to_coloring_page_job");
      }
    }

    const savedImage = localStorage.getItem("photo_to_coloring_page_image");
    if (savedImage) {
      try {
        ...
        localStorage.removeItem("photo_to_coloring_page_image");
      }
    }

    const savedFormData = localStorage.getItem("photo_to_coloring_page_form");
    if (savedFormData) {
      try {
        ...
        localStorage.removeItem("photo_to_coloring_page_form");
      }
    }

    const savedResult = localStorage.getItem("photo_to_coloring_page_result");
    if (savedResult) {
      try {
        ...
        localStorage.removeItem("photo_to_coloring_page_result");
      }
    }
  }, []);
    ...
    const pollJobStatus = async () => {
      try {
        ...
        if (result.success) {
          ...
          setJob(job);
          localStorage.setItem("photo_to_coloring_page_job", JSON.stringify(job));
  ...
  const handleEdit = useCallback(() => {
    ...
    localStorage.setItem("photo_to_coloring_page_image", JSON.stringify(imageData));
