...
const FEATURE_KEY = "photo_to_caricature"; // config/featureList.ts

export default function PhotoToCaricatureClient() {
  ...
  const t = useTranslations("PhotoToCaricature.client");

  const [sourceImage, setSourceImage] = useState<string | null>(null);
  const [sourceImageUrl, setSourceImageUrl] = useState<string | null>(null);
  const [resultImage, setResultImage] = useState<string | null>(null);
  const [prompt, setPrompt] = useState("Convert any image into enchanting Studio Ghibli-style artwork with the beloved hand-drawn animation aesthetics that have captivated audiences worldwide.");
  ...
  const saveFormData = useCallback(() => {
    ...
    localStorage.setItem("photo_to_caricature_form", JSON.stringify(formData));
  ...
  useEffect(() => {
    const savedJob = localStorage.getItem("photo_to_caricature_job");
    if (savedJob) {
      try {
        ...
        localStorage.removeItem("photo_to_caricature_job");
      }
    }

    const savedImage = localStorage.getItem("photo_to_caricature_image");
    if (savedImage) {
      try {
        ...
        localStorage.removeItem("photo_to_caricature_image");
      }
    }

    const savedFormData = localStorage.getItem("photo_to_caricature_form");
    if (savedFormData) {
      try {
        ...
        localStorage.removeItem("photo_to_caricature_form");
      }
    }

    const savedResult = localStorage.getItem("photo_to_caricature_result");
    if (savedResult) {
      try {
        ...
        localStorage.removeItem("photo_to_caricature_result");
      }
    }
  }, []);
    ...
    const pollJobStatus = async () => {
      try {
        ...
        if (result.success) {
          ...
          setJob(job);
          localStorage.setItem("photo_to_caricature_job", JSON.stringify(job));
  ...
  const handleEdit = useCallback(() => {
    ...
    localStorage.setItem("photo_to_caricature_image", JSON.stringify(imageData));
