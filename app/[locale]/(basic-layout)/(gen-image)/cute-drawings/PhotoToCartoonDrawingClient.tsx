...
const FEATURE_KEY = "photo_to_cartoon_drawing"; // config/featureList.ts

export default function PhotoToCartoonDrawingClient() {
  ...
  const t = useTranslations("PhotoToCartoonDrawing.client");

  const [sourceImage, setSourceImage] = useState<string | null>(null);
  const [sourceImageUrl, setSourceImageUrl] = useState<string | null>(null);
  const [resultImage, setResultImage] = useState<string | null>(null);
  const [prompt, setPrompt] = useState("Make this a 90s cartoon");
  ...
  const saveFormData = useCallback(() => {
    ...
    localStorage.setItem("photo_to_cartoon_drawing_form", JSON.stringify(formData));
  ...
  useEffect(() => {
    const savedJob = localStorage.getItem("photo_to_cartoon_drawing_job");
    if (savedJob) {
      try {
        ...
        localStorage.removeItem("photo_to_cartoon_drawing_job");
      }
    }

    const savedImage = localStorage.getItem("photo_to_cartoon_drawing_image");
    if (savedImage) {
      try {
        ...
        localStorage.removeItem("photo_to_cartoon_drawing_image");
      }
    }

    const savedFormData = localStorage.getItem("photo_to_cartoon_drawing_form");
    if (savedFormData) {
      try {
        ...
        localStorage.removeItem("photo_to_cartoon_drawing_form");
      }
    }

    const savedResult = localStorage.getItem("photo_to_cartoon_drawing_result");
    if (savedResult) {
      try {
        ...
        localStorage.removeItem("photo_to_cartoon_drawing_result");
      }
    }
  }, []);
    ...
    const pollJobStatus = async () => {
      try {
        ...
        if (result.success) {
          ...
          setJob(job);
          localStorage.setItem("photo_to_cartoon_drawing_job", JSON.stringify(job));
          ...
          if (job.status === "succeeded") {
            ...
            if (resultUrl) {
              ...
              localStorage.setItem(
                "photo_to_cartoon_drawing_result",
  ...
  const handleImageUpload = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        localStorage.setItem(
          "photo_to_cartoon_drawing_image",
  ...
  const handleEdit = useCallback(() => {
    ...
    localStorage.setItem("photo_to_cartoon_drawing_image", JSON.stringify(imageData));
    ...
    <section className="container mx-auto px-4 py-4 mb-8">
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-lg p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col space-y-6">
          ...
          {/* Hidden prompt input - using default value "Make this a 90s cartoon" */}
          <input
            type="hidden"
            value={prompt}
