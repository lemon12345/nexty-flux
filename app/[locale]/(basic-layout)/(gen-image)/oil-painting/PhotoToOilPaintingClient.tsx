...
const FEATURE_KEY = "photo_to_oil_painting"; // config/featureList.ts

export default function PhotoToOilPaintingClient() {
  ...
  const t = useTranslations("PhotoToOilPainting.client");

  const [sourceImage, setSourceImage] = useState<string | null>(null);
  const [sourceImageUrl, setSourceImageUrl] = useState<string | null>(null);
  const [resultImage, setResultImage] = useState<string | null>(null);
  const [prompt, setPrompt] = useState("Transform photo into oil painting style");
  ...
  const saveFormData = useCallback(() => {
    ...
    localStorage.setItem("photo_to_oil_painting_form", JSON.stringify(formData));
  ...
  useEffect(() => {
    const savedJob = localStorage.getItem("photo_to_oil_painting_job");
    if (savedJob) {
      try {
        ...
        localStorage.removeItem("photo_to_oil_painting_job");
      }
    }

    const savedImage = localStorage.getItem("photo_to_oil_painting_image");
    if (savedImage) {
      try {
        ...
        localStorage.removeItem("photo_to_oil_painting_image");
      }
    }

    const savedFormData = localStorage.getItem("photo_to_oil_painting_form");
    if (savedFormData) {
      try {
        ...
        localStorage.removeItem("photo_to_oil_painting_form");
      }
    }

    const savedResult = localStorage.getItem("photo_to_oil_painting_result");
    if (savedResult) {
      try {
        ...
        localStorage.removeItem("photo_to_oil_painting_result");
      }
    }
  }, []);
    ...
    const pollJobStatus = async () => {
      try {
        ...
        if (result.success) {
          ...
          setJob(job);
          localStorage.setItem("photo_to_oil_painting_job", JSON.stringify(job));
  ...
  const handleEdit = useCallback(() => {
    ...
    localStorage.setItem("photo_to_oil_painting_image", JSON.stringify(imageData));
    ...
    <section className="container mx-auto px-4 py-4 mb-8">
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-lg p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col space-y-6">
          ...
          {/* Hidden prompt input - using default value for oil painting generation */}
          <input
            type="hidden"
            value={prompt}
