...
const FEATURE_KEY = "photo_to_sculpture"; // config/featureList.ts

export default function PhotoToSculptureClient() {
  ...
  const t = useTranslations("PhotoToSculpture.client");

  const [sourceImage, setSourceImage] = useState<string | null>(null);
  const [sourceImageUrl, setSourceImageUrl] = useState<string | null>(null);
  const [resultImage, setResultImage] = useState<string | null>(null);
  const [prompt, setPrompt] = useState("Generate stunning marble sculptures with ultra-fine detail, shiny reflective surfaces, and professional artistic craftsmanship. Create classical art pieces with elegant lighting and mesmerizing visual effects");
  ...
  const saveFormData = useCallback(() => {
    ...
    localStorage.setItem("photo_to_sculpture_form", JSON.stringify(formData));
  ...
  useEffect(() => {
    const savedJob = localStorage.getItem("photo_to_sculpture_job");
    if (savedJob) {
      try {
        ...
        localStorage.removeItem("photo_to_sculpture_job");
      }
    }

    const savedImage = localStorage.getItem("photo_to_sculpture_image");
    if (savedImage) {
      try {
        ...
        localStorage.removeItem("photo_to_sculpture_image");
      }
    }

    const savedFormData = localStorage.getItem("photo_to_sculpture_form");
    if (savedFormData) {
      try {
        ...
        localStorage.removeItem("photo_to_sculpture_form");
      }
    }

    const savedResult = localStorage.getItem("photo_to_sculpture_result");
    if (savedResult) {
      try {
        ...
        localStorage.removeItem("photo_to_sculpture_result");
      }
    }
  }, []);
    ...
    const pollJobStatus = async () => {
      try {
        ...
        if (result.success) {
          ...
          setJob(job);
          localStorage.setItem("photo_to_sculpture_job", JSON.stringify(job));
  ...
  const handleEdit = useCallback(() => {
    ...
    localStorage.setItem("photo_to_sculpture_image", JSON.stringify(imageData));
    ...
    <section className="container mx-auto px-4 py-4 mb-8">
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-lg p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col space-y-6">
          ...
          {/* Hidden prompt input - using default value for sculpture generation */}
          <input
            type="hidden"
            value={prompt}
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-stone-500 to-gray-600 hover:from-stone-600 hover:to-gray-700 text-white py-3 rounded-lg font-medium transition-all duration-200 shadow-lg hover:shadow-xl"
            size="lg"
          >
            {isProcessing ? (
              <>
