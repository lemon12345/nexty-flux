...
const FEATURE_KEY = "photo_to_line_art"; // config/featureList.ts

export default function PhotoToLineArtClient() {
  ...
  const t = useTranslations("PhotoToLineArt.client");

  const [sourceImage, setSourceImage] = useState<string | null>(null);
  const [sourceImageUrl, setSourceImageUrl] = useState<string | null>(null);
  const [resultImage, setResultImage] = useState<string | null>(null);
  const [prompt, setPrompt] = useState("Convert the image into charming kawaii-style enamel pins with glossy metal outlines and vibrant enamel fills. Rich, saturated colors that replicate authentic enamel pin appearance.");
  ...
  const saveFormData = useCallback(() => {
    ...
    localStorage.setItem("photo_to_line_art_form", JSON.stringify(formData));
  ...
  useEffect(() => {
    const savedJob = localStorage.getItem("photo_to_line_art_job");
    if (savedJob) {
      try {
        ...
        localStorage.removeItem("photo_to_line_art_job");
      }
    }

    const savedImage = localStorage.getItem("photo_to_line_art_image");
    if (savedImage) {
      try {
        ...
        localStorage.removeItem("photo_to_line_art_image");
      }
    }

    const savedFormData = localStorage.getItem("photo_to_line_art_form");
    if (savedFormData) {
      try {
        ...
        localStorage.removeItem("photo_to_line_art_form");
      }
    }

    const savedResult = localStorage.getItem("photo_to_line_art_result");
    if (savedResult) {
      try {
        ...
        localStorage.removeItem("photo_to_line_art_result");
      }
    }
  }, []);
    ...
    const pollJobStatus = async () => {
      try {
        ...
        if (result.success) {
          ...
          setJob(job);
          localStorage.setItem("photo_to_line_art_job", JSON.stringify(job));
  ...
  const handleEdit = useCallback(() => {
    ...
    localStorage.setItem("photo_to_line_art_image", JSON.stringify(imageData));
