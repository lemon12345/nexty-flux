...
export async function POST(req: NextRequest) {
  ...
  // translate and optimize prompt(⚠️Flux Kontext only support English prompt)
  const processedParameters = { ...parameterValidation.data };

  if (processedParameters.prompt && typeof processedParameters.prompt === 'string') {
    const originalPrompt = processedParameters.prompt;
    const optimizedPrompt = await translateAndOptimizePrompt(originalPrompt);

    processedParameters.prompt = optimizedPrompt;
    processedParameters.origin_prompt = originalPrompt;
  }
  ...
  try {
    ...
    const webhookUrl = new URL(
      "/api/ai/replicate/flux-kontent/webhook",
      baseURL
    ).href;
    triggerReplicateJob(
      job.id,
